...
def basic_block(x, planes, stride=1, downsample=None, name=None):
    ...
    out = layers.BatchNormalization(
        momentum=0.9, epsilon=1e-5, name=f'{name}.bn1')(out)
    out = layers.ReLU(name=f'{name}.relu1')(out)

    out = conv3x3(out, planes, name=f'{name}.conv2')
    out = layers.BatchNormalization(
        momentum=0.9, epsilon=1e-5, name=f'{name}.bn2')(out)
...
def resnet(x, blocks_per_layer, num_classes=10):
    ...
    x = layers.Dense(units=num_classes, kernel_initializer=initializer,
...
def resnet18_ws(x, **kwargs):
    x = resnet(x, [2, 2, 2, 2], **kwargs)
    return sigmoid(x)


def generate_ResNet(input_shape=(28, 28, 1), num_classes=10):
...
def generate_ResNet_ws(input_shape=(28, 28, 1), num_classes=10):
    inputs = keras.Input(input_shape)
    outputs = resnet18_ws(inputs, num_classes=num_classes)
    model = keras.Model(inputs, outputs)
    return model
